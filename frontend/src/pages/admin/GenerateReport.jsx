...
// import { Calendar, FileText, FileCheck, Sparkles, ChevronLeft } from "lucide-react";
...
const GenerateReport = () => {
  ...
  const handleGenerate = async () => {
    ...
    // Dynamic Form Interception (Instead of a Modal Error Pop-up)
    ...
    try {
      ...
      setShowSuccess(true);
      setTouched(false); // Reset field states upon successful download initiation
    } catch (err) {
      console.error(err);
      setErrorMessage("Server error while generating report");
      setShowErrorPopup(true);
  ...
  return (
    <>
          ...
          <div className="space-y-6">
            ...
            <div ref={stepOneRef} className="bg-white p-5 sm:p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
              <div className="flex items-center gap-3">
                ...
                <h2 className="font-semibold">Choose Report Layout <span className="text-red-500">*</span></h2>
              </div>
              ...
              {touched && !type && <p className="text-xs text-red-500 font-semibold -mt-3">Report template arrangement selection is required.</p>}
              ...
              {type && (
                <div className="space-y-2">
                  ...
                  {touched && !year && <p className="text-xs text-red-500 font-semibold"> Active operational calendar timeline verification required.</p>}
                </div>
              )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      />
                      {touched && !fromDate && <p className="text-xs text-red-500 font-medium"> Start target timeframe required</p>}
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      />
                      {touched && !toDate && <p className="text-xs text-red-500 font-medium"> Concluding evaluation deadline required</p>}
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white p-5 sm:p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="relative">
                  ...
                  {open && (
                    <div className="absolute z-20 bg-white border border-gray-100 rounded-2xl mt-2 w-full shadow-xl max-h-64 overflow-y-auto p-1">
                      <label className="flex gap-2 p-2 cursor-pointer hover:bg-gray-50 rounded-lg">
                        <input type="checkbox" checked={selectedProgrammes.length === 0} onChange={() => setSelectedProgrammes([])} />
                        All Programmes
                      </label>
                      {programmes.map((p) => (
                        <label key={p._id} className="flex gap-2 p-2 cursor-pointer hover:bg-gray-50 rounded-lg">
                          <input
                            type="checkbox"
                            checked={selectedProgrammes.includes(p._id)}
                            onChange={() =>
                ...
                <div className="relative">
                  ...
                  >
                    {selectedProjects.length === 0
                      ? selectedProgrammes.length === 0 ? "All Projects (Global)" : "All Projects (Selected Programmes)"
                          ...
                          <input
                            ...
                            onChange={() =>
                              setSelectedDzongkhags((prev) =>
                                prev.includes(dz) ? prev.filter((item) => item !== dz) : [...prev, dz]
                              )
                            }
                          />
      ...
      {/* BACKEND ERROR TRANSACTION POPUP */}
      {showErrorPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-[100] px-4">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm"></div>
          <div className="relative bg-white rounded-xl shadow-2xl px-6 sm:px-10 py-8 sm:py-10 text-center w-full max-w-xl animate-popup">
