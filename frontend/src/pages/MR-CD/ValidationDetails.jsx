...
const ValidationDetails = () => {
  ...
  useEffect(() => {
    const fetchFullDetails = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API_URL}/api/projects/summary/${id}`, {
        ...
        if (json.success) {
          ...
          // Map rows directly from geographicBreakdown processing
          const rows = (json.geographicBreakdown || []).flatMap(geo => 
            geo.activities
              ...
              .map(act => ({
                ...
                // FIXED: Extracting the calculated actual quantity cleanly from the breakdown object
                realQuantity: act.realQuantity !== undefined ? act.realQuantity : 0
              }))
          );
          setGeoRows(rows);
        }
...
const handleSaveQuantities = async () => {
    try {
      // Group values into the nested location-based bulk update structure
      const groupedMap = {};

      geoRows.forEach((r, i) => {
        const key = `${r.dzongkhag}_${r.gewog}_${r.village}`;
        if (!groupedMap[key]) {
          groupedMap[key] = {
            location: {
              dzongkhag: r.dzongkhag,
              gewog: r.gewog || "",
              village: r.village
            },
            activities: []
      ...
      const bulkVerifications = Object.values(groupedMap);

      const res = await fetch(`${API_URL}/api/projects/${id}/verify-data`, {
        method: "POST",
        headers: { "Content-Type": "application/json", 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({ bulkVerifications })
      });
      ...
      if (res.ok) {
        ...
        // OPTIMIZATION: Manually sync local rows state to avoid window.location.reload()
        setGeoRows(prevRows => 
          prevRows.map((row, i) => ({
            ...row,
            realQuantity: tempActuals[i] !== undefined ? Number(tempActuals[i]) : row.realQuantity
          }))
        );

        // Automatically hide the success banner alert without hard resetting the scroll layout
  ...
  const handleSendAlert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/projects/${id}/alert`, {
        method: "POST",
        headers: { 
        ...
        body: JSON.stringify({ 
          ...
          remark: remarkText.trim() || "No additional remarks.",
        ...
        <div className="bg-white rounded-xl p-8 border border-gray-100 shadow-sm w-full">
          ...
          <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-5 mb-10 w-full">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-800 tracking-tight">{projectData?.projectName}</h2>
          ...
          <div className="w-full overflow-x-auto min-w-0">
            <table className="min-w-[800px] w-full text-left border-separate border-spacing-y-2">
              <thead>
                <tr className="text-gray-800 capitalize font-bold text-sm">
              ...
              <tbody className="text-sm font-medium">
                {paginatedRows.map((row, i) => {
                  ...
                  return (
                    <tr key={globalIndex} className="bg-white hover:bg-gray-50/50 transition-colors border-y border-gray-50">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-xl p-6 sm:p-10 ">
            <h3 className="text-xl font-bold text-gray-800 mb-6">Synchronize Actuals</h3>
              ...
              {geoRows.map((item, i) => {
                ...
                return (
                  <div key={i} className={`flex justify-between items-center p-4 rounded-xl border transition-colors ${isMismatch ? 'bg-red-50 border-red-100' : 'bg-gray-50 border-transparent'}`}>
                    <span className={`text-[10px] font-bold uppercase ${isMismatch ? 'text-red-500' : 'text-gray-700'}`}>
                    ...
                    <input 
                      ...
                      className={`w-20 rounded-xl py-2 text-center font-bold outline-none shadow-sm border ${isMismatch ? 'bg-white border-red-200 text-red-600 focus:ring-red-400' : 'bg-white border-gray-100 text-blue-600 focus:ring-blue-400'}`}
                    />
                  </div>
                );
              })}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-2xl p-6 sm:p-10 shadow-xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-bold text-gray-900">Report Discrepancy</h3>
              </div>
