...
//   // State for Live Validation Error Tracking
//   const [fieldErrors, setFieldErrors] = useState({});
//   const [showErrorPopup, setShowErrorPopup] = useState(false);
//   const [errorMessage, setErrorMessage] = useState("");

//   // Base state model for dynamic location assignment
...
//   // Array of beneficiaries profiles (Starts with exactly 1 table row by default)
//   const [beneficiaries, setBeneficiaries] = useState([
//     { name: '', cid: '', gender: '', houseNo: '', thramNo: '', indirectMale: 0, indirectFemale: 0 }
//   ]);

//   // Shared Key Activities model for this submission batch
...
//   // 📡 FETCH PROJECTS USING AUTH HEADERS
//   useEffect(() => {
...
//   // --- Regional Content Input Handler ---
...
//   // --- Beneficiary Array Profile Handlers ---
...
//     // 1. Check for Active Live Error Flags
...
//     // 3. Beneficiaries Iteration Checklist
...
//     try {
//       // Map Shared Interventions matching target backend specifications
...
//       // Construct calls for single payload configuration sequences or bulk maps execution sequences
//       const promises = beneficiaries.map(b => {
//         const payload = {
...
//         };
//         return axios.post("http://localhost:5000/api/beneficiaries", payload, {
//           headers: { Authorization: `Bearer ${token}` }
//         });
//       });
...
//             {/* ─── SECTION 1: REGIONAL & PROJECT IDENTIFICATION ─── */}
...
//                   <select required className={`w-full p-2.5 border text-sm rounded-lg outline-none appearance-none bg-white ${getTextColor(regionalData.projectId)}`}
...
//                   <select required className={`w-full p-2.5 border text-sm rounded-lg outline-none appearance-none bg-white capitalize ${getTextColor(regionalData.dzongkhag)} ${fieldErrors.dzongkhag ? 'border-red-400' : ''}`} 
...
//                   </select>
//                   <ChevronDown className="absolute right-3 top-[32px] text-gray-400 pointer-events-none" size={16} />
//                   {fieldErrors.dzongkhag && <p className="text-[10px] text-red-500 mt-0.5">{fieldErrors.dzongkhag}</p>}
//                 </div>
...
//             {/* ─── SECTION 2: TABLE FLOW PROFILE METRICS MATRIX ─── */}
//             <div className="bg-gray-50/50 p-6 rounded-xl border border-gray-100 space-y-6">
...
//                         <td className="p-2 vertical-align-top">
//                           <input required type="text" placeholder="Name" className={`w-full p-2 text-xs border rounded-lg outline-none bg-white ${fieldErrors[`name-${bIdx}`] ? 'border-red-400 bg-red-50/30' : ''}`}
//                             value={b.name} onChange={e => handleBeneficiaryChange(bIdx, 'name', e.target.value)} />
//                           {fieldErrors[`name-${bIdx}`] && <p className="text-[9px] text-red-500 mt-0.5 leading-tight">{fieldErrors[`name-${bIdx}`]}</p>}
//                         </td>
//                         <td className="p-2">
//                           <input required type="text" maxLength={11} placeholder="11-digit CID" className={`w-full p-2 text-xs border rounded-lg outline-none bg-white ${fieldErrors[`cid-${bIdx}`] ? 'border-red-400 bg-red-50/30' : ''}`}
//                             value={b.cid} onChange={e => handleBeneficiaryChange(bIdx, 'cid', e.target.value)} />
//                           {fieldErrors[`cid-${bIdx}`] && <p className="text-[9px] text-red-500 mt-0.5 leading-tight">{fieldErrors[`cid-${bIdx}`]}</p>}
//                         </td>
//                         <td className="p-2 relative">
//                           <select required className={`w-full p-2 pr-6 border text-xs rounded-lg outline-none appearance-none bg-white ${getTextColor(b.gender)}`}
...
//                         </td>
//                         <td className="p-2">
//                           <input required type="text" placeholder="H No." className="w-full p-2 text-xs border rounded-lg outline-none bg-white"
//                             value={b.houseNo} onChange={e => handleBeneficiaryChange(bIdx, 'houseNo', e.target.value)} />
//                         </td>
//                         <td className="p-2">
//                           <input required type="text" placeholder="Thram Designation" className="w-full p-2 text-xs border rounded-lg outline-none bg-white"
//                             value={b.thramNo} onChange={e => handleBeneficiaryChange(bIdx, 'thramNo', e.target.value)} />
//                         </td>
//                         <td className="p-2">
//                           <input type="number" min="0" placeholder="0" className="w-full p-2 text-xs border rounded-lg outline-none bg-white"
...
//             {/* ─── SECTION 3: INTERVENTION TARGET PLAN ─── */}
...
//                       <span className="text-xs font-bold uppercase text-gray-990">Key Activity</span>
//                     </label>
//                     <label className="flex items-center gap-2 cursor-pointer">
...
//               <Save size={20}/> Review and Save Profiles ({beneficiaries.length})
//             </button>
//           </form>
//         </div>
//       </div>
...
//           <div className="bg-white rounded-xl p-8 max-w-xl w-full shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
//             <h3 className="text-xl font-bold text-gray-800 border-b pb-2">Confirm Beneficiary Profile Submission</h3>
//             <div className="space-y-4 text-sm text-gray-600 bg-gray-50 p-5 rounded-2xl border">
...
//               <p><strong>Location:</strong> <span className="text-black font-medium capitalize">{regionalData.dzongkhag}, {regionalData.gewog}, {regionalData.village}</span></p>
//               <p><strong>Total Beneficiaries to Register:</strong> <span className="text-blue-600 font-bold">{beneficiaries.length} profile(s)</span></p>
              
//               <div className="border-t pt-3 mt-2 max-h-[200px] overflow-y-auto space-y-2">
//                 <p className="font-bold text-xs text-gray-400 uppercase tracking-wider">Profile Listing Preview:</p>
//                 {beneficiaries.map((b, idx) => (
//                   <p key={idx} className="text-xs text-gray-700 bg-white p-2 rounded-lg border border-gray-100">
//                     Row {idx+1}: <strong>{b.name || 'Unnamed'}</strong> (CID: {b.cid || 'N/A'})
//                   </p>
//                 ))}
//               </div>
//             </div>
//             <div className="flex gap-4">
//               <button onClick={() => setShowConfirm(false)} className="flex-1 p-3 border rounded-xl font-bold text-gray-500 hover:bg-gray-100 transition-all">Go Back</button>
//               <button onClick={handleSubmit} className="flex-1 p-3 bg-[#2EA1F2] text-white rounded-xl font-bold hover:bg-[#298CD2] transition-all">Confirm & Save All</button>
//             </div>
//           </div>
//         </div>
//       )}

//       <SuccessModal isOpen={isSuccess} onClose={() => setIsSuccess(false)} message={`${beneficiaries.length} Beneficiary Profile(s) Saved Successfully`} />
